...
class TestBacklinkStatisticsWorker:
    ...
    @pytest.mark.asyncio
    async def test_store_statistics_success(self):
        """Test storing statistics successfully."""
        mock_vitess_client = MagicMock()
        ...
        stats = MagicMock(
            total_backlinks=100,
            unique_entities_with_backlinks=50,
            top_entities_by_backlinks=[{"entity": "Q1", "count": 10}]
        )

        with patch('models.workers.backlink_statistics.backlink_statistics_worker.date') as mock_date:
    ...
    async def test_store_statistics_no_vitess_client(self):
        """Test storing statistics with no vitess client."""
        worker = BacklinkStatisticsWorker(vitess_client=None)
