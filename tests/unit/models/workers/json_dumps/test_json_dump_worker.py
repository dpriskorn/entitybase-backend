    ...
    def test_calculate_seconds_until_next_run(self):
        ...
        with patch(
            ...
            with patch(
                ...
                # Mock time, combine, and timedelta
                ...
                # Should be seconds from 10 AM to 2 AM next day (16 hours = 57600 seconds)
                assert seconds > 0
        ...
        with patch("asyncio.sleep", side_effect=mock_sleep):
            with patch(
                ...
                with patch.object(WorkerClass, "lifespan", mock_lifespan):
                    with patch.object(
                        worker, "_calculate_seconds_until_next_run", return_value=0.001
                    ):
                        with patch.object(
                            WorkerClass,
                            "run_weekly_dump",
        ...
        with patch.object(worker, "_fetch_all_entities", return_value=[]):
            with patch.object(worker, "_fetch_entities_for_week", return_value=[]):
                with patch.object(worker, "_generate_and_upload_dump") as mock_generate:
                    ...
                    full_dump_call = [
                        c for c in mock_generate.call_args_list if "full" in str(c)
                    ]
    ...
    async def test_upload_to_s3_with_checksum(self):
        ...
        with tempfile.NamedTemporaryFile(mode="w", delete=False, suffix=".json") as f:
            ...
            with patch(
                ...
                await worker._upload_to_s3(
                    filepath, "weekly/2025-01-15/full.json", checksum
                )
