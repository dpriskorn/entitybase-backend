...
class TestUserRepository:
    ...
    def test_log_user_activity_invalid_activity_type(self):
        """Test logging activity with invalid activity type."""
        mock_vitess_client = MagicMock()

        repo = UserRepository(vitess_client=mock_vitess_client)

        result = repo.log_user_activity(123, None, "Q1", 1)
    ...
    def test_get_user_activities_with_activity_type(self):
        """Test getting activities with activity type filter."""
        mock_vitess_client = MagicMock()
        mock_cursor = MagicMock()
        mock_cursor.__enter__ = MagicMock(return_value=mock_cursor)
        mock_cursor.__exit__ = MagicMock(return_value=False)
        mock_cursor.fetchall.return_value = [
            (1, 123, "entity_edit", "Q1", 1, "2023-01-01"),
        ]
        mock_vitess_client.cursor = mock_cursor

        repo = UserRepository(vitess_client=mock_vitess_client)

        result = repo.get_user_activities(123, activity_type=UserActivityType.ENTITY_EDIT)

        assert result.success is True
        assert len(result.data) == 1
        assert result.data[0].activity_type == "entity_edit"
