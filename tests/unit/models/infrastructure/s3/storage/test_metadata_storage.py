...
class TestMetadataStorage:
    ...
    def test_get_bucket_for_type_terms(self) -> None:
        ...
        with patch(
            "models.infrastructure.s3.storage.metadata_storage.settings"
        ) as mock_settings:
            mock_settings.s3_terms_bucket = "test-terms"
            mock_settings.s3_sitelinks_bucket = "test-sitelinks"

            assert (
                MetadataStorage._get_bucket_for_type(MetadataType.LABELS)
                == "test-terms"
            )
            assert (
                MetadataStorage._get_bucket_for_type(MetadataType.DESCRIPTIONS)
                == "test-terms"
            )
            assert (
                MetadataStorage._get_bucket_for_type(MetadataType.ALIASES)
                == "test-terms"
            )
            assert (
                MetadataStorage._get_bucket_for_type(MetadataType.FORM_REPRESENTATIONS)
                == "test-terms"
            )
            assert (
                MetadataStorage._get_bucket_for_type(MetadataType.SENSE_GLOSSES)
                == "test-terms"
            )
    ...
    def test_get_bucket_for_type_sitelinks(self) -> None:
        ...
        with patch(
            ...
            assert (
                MetadataStorage._get_bucket_for_type(MetadataType.SITELINKS)
                == "test-sitelinks"
            )
    ...
    def test_load_metadata_not_found(self) -> None:
        ...
        with patch(
            "models.infrastructure.s3.base_storage.BaseS3Storage.load",
            return_value=None,
        ) as mock_load:
            with patch(
                "models.infrastructure.s3.storage.metadata_storage.settings"
    ...
    def test_delete_metadata_success(self) -> None:
        ...
        with patch(
            "models.infrastructure.s3.base_storage.BaseS3Storage.delete",
