...
Note: X-User-ID has been removed from the model. User identity is now
determined by authentication via the Authorization header (JWT).
"""
...
class TestEditHeaders:
    ...
    def test_maximum_summary_length(self):
        ...
        edit_headers = EditHeaders(x_edit_summary="a" * 200)
    ...
    def test_too_long_summary_fails(self):
        ...
        with pytest.raises(ValidationError):
            EditHeaders(x_edit_summary="a" * 201)

    def test_alias_instantiation(self):
        """Test instantiation using kebab-case aliases."""
        edit_headers = EditHeaders(x_edit_summary="Alias test")
    ...
    def test_base_revision_id_negative_fails(self):
        ...
        with pytest.raises(ValidationError):
            EditHeaders(x_edit_summary="test", x_base_revision_id=-1)

    def test_model_dump_with_base_revision_id(self):
        """Test model_dump() includes base_revision_id."""
        edit_headers = EditHeaders(x_edit_summary="test", x_base_revision_id=42)
        dumped = edit_headers.model_dump()
        assert dumped == {
            "x_edit_summary": "test",
            "x_base_revision_id": 42,
