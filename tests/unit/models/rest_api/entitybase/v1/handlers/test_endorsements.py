...
class TestEndorsementHandler:
    ...
    def mock_handler(self, mock_state):
        ...
        with patch(
            "models.rest_api.entitybase.v1.handlers.endorsements.EndorsementHandler"
        ) as MockClass:
    ...
    def test_handle_endorsement_error_not_found(self, mock_handler):
        ...
        with pytest.raises(HTTPException) as exc_info:
            EndorsementHandler._handle_endorsement_error(
                mock_handler, error_msg, "create"
            )
    ...
    @pytest.mark.asyncio
    async def test_publish_endorsement_event_without_producer(
    ...
    def test_find_endorsement_by_user_active(self, mock_handler, mock_endorsement):
        ...
        result = EndorsementHandler._find_endorsement_by_user(
            mock_handler, endorsements, 1, must_be_active=True
        )
    ...
    def test_find_endorsement_by_user_withdrawn(self, mock_handler, mock_endorsement):
        ...
        result = EndorsementHandler._find_endorsement_by_user(
            mock_handler, endorsements, 1, must_be_active=False
        )
    ...
    def test_find_endorsement_by_user_not_found(self, mock_handler, mock_endorsement):
        ...
        with pytest.raises(HTTPException) as exc_info:
            EndorsementHandler._find_endorsement_by_user(
                mock_handler, endorsements, 999, must_be_active=True
            )
    ...
    def test_get_and_validate_endorsement_success(
        self, mock_handler, mock_state, mock_endorsement
    ):
        ...
        mock_state.mysql_client.endorsement_repository.get_statement_endorsements.return_value = mock_result

        # Verify that the repository is called and data is retrieved
        result = EndorsementHandler._get_and_validate_endorsement(
            mock_handler, 12345, 1, must_be_active=True
        )

        # Just verify we got a result (the mock endorsement is returned)
        mock_state.mysql_client.endorsement_repository.get_statement_endorsements.assert_called_once()
