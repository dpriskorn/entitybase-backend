...
class TestLexemeCreateHandler:
    ...
    def _make_handler(self, mock_state: MagicMock) -> LexemeCreateHandler:
        handler = LexemeCreateHandler.model_construct(
            state=mock_state, enumeration_service=MagicMock()
        )
        return handler
    ...
    @pytest.mark.asyncio
    async def test_create_lexeme_no_lemmas_raises_error(
    ...
    @pytest.mark.asyncio
    async def test_create_lexeme_invalid_language_raises_error(
        self, mock_state: MagicMock
    ) -> None:
        handler = self._make_handler(mock_state)
    ...
    @pytest.mark.asyncio
    async def test_create_lexeme_invalid_lexical_category_raises_error(
        self, mock_state: MagicMock
    ) -> None:
        handler = self._make_handler(mock_state)
