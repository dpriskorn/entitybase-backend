...
class TestStatementServiceSnakIntegration:
    ...
    def test_deduplicate_statements_extracts_and_stores_snaks(
        self, mock_state, sample_hash_result
    ):
        """Test that mainsnak is extracted and stored via SnakHandler."""
        ...
        with patch('models.rest_api.entitybase.v1.services.statement_service.SnakHandler') as MockSnakHandler:
            ...
            mock_snak_handler_instance.store_snak.return_value = 987654321
            ...
            snak_call = mock_snak_handler_instance.store_snak.call_args[0][0]
            assert snak_call.property == "P31"
            assert "datavalue" in snak_call.datavalue
            
        assert result.success is True
    
    def test_deduplicate_statements_with_qualifiers(
        self, mock_state
    ):
        """Test that qualifiers are deduplicated with snak hashes."""
        statement_data = {
            "type": "statement",
            "id": "Q123$2",
            "rank": "normal",
        ...
        with patch('models.rest_api.entitybase.v1.services.statement_service.SnakHandler') as MockSnakHandler:
            mock_snak_handler_instance = Mock()
            mock_snak_handler_instance.store_snak.return_value = 555555555
            MockSnakHandler.return_value = mock_snak_handler_instance
            
            result = service.deduplicate_and_store_statements(hash_result)
            
            # Verify multiple snaks were stored (mainsnak + qualifier snaks)
            assert mock_snak_handler_instance.store_snak.call_count > 1
            
        assert result.success is True
    
    def test_deduplicate_statements_with_references(
        self, mock_state
    ):
        """Test that references are deduplicated with snak hashes."""
        statement_data = {
            "type": "statement",
            "id": "Q123$3",
            "rank": "normal",
        ...
        with patch('models.rest_api.entitybase.v1.services.statement_service.SnakHandler') as MockSnakHandler:
            mock_snak_handler_instance = Mock()
            mock_snak_handler_instance.store_snak.return_value = 666666666
            MockSnakHandler.return_value = mock_snak_handler_instance
            
            result = service.deduplicate_and_store_statements(hash_result)
            
            # Verify snaks in references were also stored
            assert mock_snak_handler_instance.store_snak.call_count > 1
