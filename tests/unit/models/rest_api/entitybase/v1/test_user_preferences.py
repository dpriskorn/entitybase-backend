...
class TestUserPreferencesHandler:
    ...
    def test_get_preferences_success(
        self, handler: UserPreferencesHandler, mock_mysql_client: MagicMock
    ) -> None:
        """Test getting user preferences successfully"""
        mock_mysql_client.user_repository.user_exists.return_value = True
        from models.data.common import OperationResult

        mock_mysql_client.user_repository.get_user_preferences.return_value = (
            OperationResult(
                success=True,
                data={
    ...
    async def test_update_preferences_success(
        ...
        mock_mysql_client.user_repository.update_user_preferences.assert_called_once_with(
