...
class TestRemoveStatement(unittest.TestCase):
    ...
    def test_statement_not_found(self, mock_read_handler_class):
        ...
        mock_entity_response.entity_data = {"claims": {"P1": [{"mainsnak": {"property": "P1"}}]}}
        ...
        result = self.handler.remove_statement("Q1", "999", request.edit_summary, self.mock_vitess, self.mock_s3)
    ...
    @patch('models.rest_api.entitybase.handlers.entity.base.VitessClient.update_head_revision')
    def test_successful_remove_statement(self, mock_update_head, mock_store_revision, mock_stmt_repo_class, mock_read_revision):
        """Test successful statement removal."""
        # Mock revision data
        from models.s3_models import RevisionData, HashMaps, StatementsHashes, PropertyCounts
        ...
        request = RemoveStatementRequest(edit_summary="Remove statement")
        result = self.handler.remove_statement("Q1", "123", request.edit_summary, self.mock_vitess, self.mock_s3)
    ...
    @patch('models.rest_api.entitybase.handlers.entity.base.MyS3Client.read_revision')
    @patch('models.rest_api.entitybase.handlers.entity.base.StatementRepository')
    def test_decrement_ref_count_failure(self, mock_stmt_repo_class, mock_read_revision):
        ...
        mock_stmt_repo = MagicMock()
        mock_stmt_repo_class.return_value = mock_stmt_repo
        mock_stmt_repo.decrement_ref_count.return_value = MagicMock(success=False, error="DB error")
        
        request = RemoveStatementRequest(edit_summary="Remove statement")
        with self.assertRaises(Exception):  # raise_validation_error raises HTTPException
            self.handler.remove_statement("Q1", "123", request.edit_summary, self.mock_vitess, self.mock_s3)
    ...
    def test_invalid_statement_hash_format(self, mock_read_revision):
        ...
        request = RemoveStatementRequest(edit_summary="Remove statement")
        result = self.handler.remove_statement("Q1", "invalid", request.edit_summary, self.mock_vitess, self.mock_s3)
