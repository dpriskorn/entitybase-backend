...
class TestPatchStatement(unittest.TestCase):
    ...
    def test_statement_hash_not_found(self):
        ...
        request = PatchStatementRequest(claim={"mainsnak": {"property": "P1"}}, edit_summary="Patch")
        
        with patch('models.rest_api.entitybase.handlers.entity.base.EntityReadHandler') as mock_read_handler_class:
            ...
            result = self.handler.patch_statement("Q1", "999", request, self.mock_vitess, self.mock_s3)
    ...
    @patch('models.internal_representation.statement_hasher.StatementHasher.compute_hash')
    def test_successful_patch_statement(self, mock_hash, mock_process, mock_create, mock_read_handler_class):
        ...
        result = self.handler.patch_statement("Q1", "123", request, self.mock_vitess, self.mock_s3)
        
        self.assertTrue(result.success)
        self.assertEqual(result.data, {"revision_id": 101})
        mock_create.assert_called_once()
        # Verify claim was replaced
        self.assertEqual(mock_entity_response.entity_data["claims"]["P1"][0], request.claim)
