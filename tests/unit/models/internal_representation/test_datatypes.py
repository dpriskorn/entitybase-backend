...
class TestDatatypes:
    ...
    def test_normalize_from_sparql_logging(self, caplog):
        """Test that normalize_from_sparql logs debug message."""
        import logging
        caplog.set_level(logging.DEBUG)

        result = Datatype.normalize_from_sparql("WikibaseItem")

        assert result == "wikibase-item"
        assert "Normalizing datatype from SPARQL: WikibaseItem" in caplog.text
