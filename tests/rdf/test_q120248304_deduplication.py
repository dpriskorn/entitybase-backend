...
def test_q120248304_no_duplicate_value_nodes(full_property_registry):
    ...
    logger.info(f"Q42: {len(wdv_blocks)} wdv blocks written (should equal unique values)")
    ...
    if converter.dedupe:
        ...
        assert stats['hits'] > 0, f"Deduplication prevented {stats['hits']} duplicate block writes"
...
def test_deduplication_disabled(full_property_registry):
    ...
    converter = EntityConverter(
        property_registry=full_property_registry,
...
def test_q42_no_duplicate_value_nodes(full_property_registry):
    ...
    # Count all wdv: blocks
    wdv_ids = re.findall(r'wdv:([a-f0-9]{32})', actual_ttl)
    unique_ids = set(wdv_ids)
    
    logger.info(f"Q42: {len(wdv_ids)} total wdv: refs, {len(unique_ids)} unique")
