...
class TestQualifierDeduplication:
    ...
    def test_qualifier_endpoint_single_fetch(self, api_client, base_url) -> None:
    ...
    def test_qualifier_endpoint_batch_fetch(self, api_client, base_url) -> None:
        ...
        # Test batch fetch
        response = api_client.get(f"{base_url}/entitybase/v1/qualifiers/{hashes_str}")
        assert response.status_code == 200
        result = response.json()
    ...
    def test_qualifier_endpoint_invalid_hash(self, api_client, base_url) -> None:
        """Test qualifier endpoint with invalid hash format."""
        response = api_client.get(f"{base_url}/entitybase/v1/qualifiers/invalid-hash")
        assert response.status_code == 400
        error = response.json()
        assert "Invalid hash format" in error["detail"]

    def test_qualifier_endpoint_too_many_hashes(self, api_client, base_url) -> None:
        ...
        response = api_client.get(f"{base_url}/entitybase/v1/qualifiers/{mock_hashes}")
        assert response.status_code == 400
        error = response.json()
        assert "Too many hashes" in error["detail"]

    def test_qualifier_endpoint_no_hashes(self, api_client, base_url) -> None:
        """Test qualifier endpoint with no hashes provided."""
        response = api_client.get(f"{base_url}/entitybase/v1/qualifiers/")
