...
def test_entity_property_hashes(api_client: requests.Session, base_url: str) -> None:
    ...
    api_client.post(f"{base_url}/entity", json=entity_data)
    raw = api_client.get(f"{base_url}/raw/Q80008/1").json()
    p31_hash = raw["statements"][0]
    p279_hash = raw["statements"][1]

    response = api_client.get(f"{base_url}/entity/Q80008/properties/P31/hashes")
    assert response.status_code == 200

    result = response.json()
    assert "hashes" in result
    assert len(result["hashes"]) == 1
    assert result["hashes"][0] == p31_hash

    response = api_client.get(f"{base_url}/entity/Q80008/properties/P279/hashes")
    assert response.status_code == 200

    result = response.json()
    assert "hashes" in result
    assert len(result["hashes"]) == 1
    assert result["hashes"][0] == p279_hash
