...
sys.path.insert(0, '/home/dpriskorn/src/python/wikibase-backend/src')
...
TEST_DATA_DIR = Path('/home/dpriskorn/src/python/wikibase-backend/test_data')
...
def debug_q42():
    ...
    for hash_val in sorted(set(duplicates))[:10]:  # Show first 10
        count = ref_counts[hash_val]
        print(f"  wdv:{hash_val} - {count} times")
    
    print(f"\nTotal duplicate hashes: {len(set(duplicates))}")
    print(f"Total duplicate references: {sum(ref_counts.values()) - len(set(duplicates))}")
