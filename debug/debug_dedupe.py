...
def debug_q42():
    ...
    for hash_val in sorted(duplicates)[:10]:  # Show first 10
        count = ref_counts[hash_val]
        print(f"  wdv:{hash_val} - {count} times")
    
    print(f"\nTotal duplicate hashes: {len(duplicates)}")
    print(f"Total duplicate references: {sum(ref_counts.values()) - len(set(wdv_refs))}")
