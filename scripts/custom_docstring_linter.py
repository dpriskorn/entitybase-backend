...
def check_docstring_format(docstring: str, node: ast.AST) -> List[str]:  # type: ignore[misc]
    ...
    if lines:
        ...
        if not first_line.endswith(('.', '!', '?')):
            errors.append("D415 First line should end with a period, question mark, or exclamation point")
    ...
    if num_lines > 1:
        ...
        if summary_end + 1 < num_lines and lines[summary_end + 1].strip():
            errors.append("D205 1 blank line required between summary line and description")
    ...
    if CHECK_GOOGLE_STYLE_ARGS_RETURNS:
        ...
        if isinstance(node, ast.FunctionDef):
            ...
            if params and not has_args_section:
                errors.append("Google-style: Missing 'Args:' section for function with parameters")
            if node.returns and not has_returns_section:
                errors.append("Google-style: Missing 'Returns:' section for function with return annotation")
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.ClassDef):
            ...
            if not node.name.startswith('_'):
                ...
                if not doc:
                    errors.append(f"{file_path}:{node.lineno}:1: D101 Missing docstring in public class '{node.name}'")
                else:
            ...
            if parent and isinstance(parent, ast.ClassDef) and parent.name in skipped_classes:
                continue
            # D103: Missing docstring in public function
            if not node.name.startswith('_'):
                doc = get_docstring(node)  # type: ignore[arg-type]
                if not doc:
                    kind = "method" if parent and isinstance(parent, ast.ClassDef) else "function"
                    code = "D102" if kind == "method" else "D103"
                    errors.append(f"{file_path}:{node.lineno}:1: {code} Missing docstring in public {kind} '{node.name}'")
                else:
                    format_errors = check_docstring_format(doc, node)
                    for err in format_errors:
                        errors.append(f"{file_path}:{node.lineno}:1: {err}")
