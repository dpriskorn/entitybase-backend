...
# Assuming rustfs or S3 compatible
aws s3 mb "s3://${REFERENCES_BUCKET}" --endpoint-url="${S3_ENDPOINT:-http://rustfs:9000}" || echo "Bucket ${REFERENCES_BUCKET} already exists or failed to create"
aws s3 mb "s3://${QUALIFIERS_BUCKET}" --endpoint-url="${S3_ENDPOINT:-http://rustfs:9000}" || echo "Bucket ${QUALIFIERS_BUCKET} already exists or failed to create"
