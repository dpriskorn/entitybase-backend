...
def fetch_property_metadata(properties_csv: Path, output_dir: Path) -> None:
    ...
    property_ids_to_download = [pid for pid in property_ids if pid not in existing_properties]
    
    if len(property_ids_to_download) < len(property_ids):
        print(f"Skipping {len(property_ids) - len(property_ids_to_download)} already downloaded")
