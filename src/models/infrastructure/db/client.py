"""MySQL-compatible database client for entity operations."""
...
from models.infrastructure.db.connection import (
...
class MysqlClient(Client):
    """MySQL-compatible database client for entity operations with connection pooling."""

    connection_manager: Optional[MysqlConnectionManager] = Field(
    ...
    def model_post_init(self, context: Any) -> None:
        logger.info("=== MysqlClient.model_post_init() START ===")
        logger.debug(
            f"Initializing MysqlClient with host='{self.config.host}', port={self.config.port}, database='{self.config.database}', user='{self.config.user}', password_length={len(self.config.password)}"
        )
        logger.debug("Creating MysqlConnectionManager...")
        self.connection_manager = MysqlConnectionManager(config=self.config)
        logger.debug("MysqlConnectionManager created, calling connect()...")
        self.connection_manager.connect()
        logger.debug("Connection established, creating IdResolver...")
        self.id_resolver = IdResolver(db_client=self)
        logger.info("=== MysqlClient.model_post_init() END ===")
    ...
    def entity_repository(self) -> Any:
        ...
        from models.infrastructure.db.repositories.entity import EntityRepository
    ...
    def thanks_repository(self) -> Any:
        ...
        from models.infrastructure.db.repositories.thanks import ThanksRepository
    ...
    def create_tables(self) -> None:
        ...
        schema_repository: SchemaRepository = SchemaRepository(db_client=self)
