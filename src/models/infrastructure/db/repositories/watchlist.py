...
class WatchlistRepository(Repository):
    ...
    def add_watch(
        ...
        try:
            internal_entity_id = self.db_client.id_resolver.resolve_id(entity_id)
    ...
    def remove_watch(
        ...
        with self._get_conn() as _:
            with self.db_client.cursor as cursor:
                internal_entity_id = self.db_client.id_resolver.resolve_id(
                    entity_id
                )
    ...
    def get_watches_for_user(self, user_id: int) -> List[Any]:
        ...
        for row in rows:
            ...
            entity_id = self.db_client.id_resolver.resolve_entity_id(
                internal_entity_id
            )
    ...
    def _get_conn(self) -> pymysql.Connection:
        ...
        return self.db_client.connection_manager.connect()  # type: ignore[no-any-return]
