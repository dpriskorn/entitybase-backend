...
class WatchlistRepository(Repository):
    ...
    def add_watch(
        ...
        try:
            internal_entity_id = self.mysql_client.id_resolver.resolve_id(entity_id)
    ...
    def get_watches_for_user(self, user_id: int) -> List[Any]:
        ...
        for row in rows:
            ...
            entity_id = self.mysql_client.id_resolver.resolve_entity_id(
                internal_entity_id
            )
    ...
    def _get_conn(self) -> pymysql.Connection:
        ...
        return self.mysql_client.connection_manager.connect()  # type: ignore[no-any-return]
