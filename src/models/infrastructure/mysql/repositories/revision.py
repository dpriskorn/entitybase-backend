"""Mysql revision repository for revision operations."""
...
from models.data.infrastructure.mysql.records.revision import (
...
class RevisionRepository(Repository):
    ...
    def get_history(
        self,
        entity_id: str = Field(
            ..., min_length=2, description="Entity ID e.g. Q1 or L1"
        ),
        limit: int = Field(default=20, description="Limit number of history items"),
        ...
        internal_id = self.mysql_client.id_resolver.resolve_id(entity_id)
        if not internal_id:
        ...
        with self.mysql_client.cursor as cursor:
            ...
            logger.debug(
                f"Retrieved {len(result)} history items for entity {entity_id}"
            )
            return result
    ...
    def delete(
        ...
        with self.mysql_client.cursor as cursor:
            ...
            logger.debug(
                f"Successfully deleted revision {revision_id} for entity {entity_id}"
            )
    ...
    def create_with_cas(
        ...
        statements = (
            ...
            if entity_data.hashes and entity_data.hashes.statements
            else "[]"
        )
        ...
        with self.mysql_client.cursor as cursor:
            cursor.execute(
                ...
                (
                    ...
                    entity_data.property_counts.model_dump_json()
                    if entity_data.property_counts
                    else "{}",
            ...
            logger.debug(
                f"CAS operation: affected {affected_rows} rows for revision {revision_id}"
            )
    ...
    def create(
        ...
        internal_id = self.mysql_client.id_resolver.resolve_id(entity_id)
        logger.debug(
            f"[RevisionRepository.create] entity_id={entity_id}, resolved internal_id={internal_id}"
        )
        if not internal_id:
            logger.error(
                f"[RevisionRepository.create] FAILED: No internal_id found for entity {entity_id}"
            )
