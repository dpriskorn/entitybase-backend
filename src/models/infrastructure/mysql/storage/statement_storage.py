"""Statement storage operations using Mysql."""
...
class StatementMysqlStorage(BaseMysqlStorage):
    """Storage operations for statements using Mysql."""
    ...
    def store_statement(
        ...
        logger.debug(f"[STMT_MYSQL_STORE] hash={content_hash}")
        result = self._store(content_hash, statement_data)
        if result.success:
            logger.debug(f"[STMT_MYSQL_STORE] SUCCESS: hash={content_hash}")
        else:
            logger.error(f"[STMT_MYSQL_STORE] FAILED: {result.error}")
        return result

    def load_statement(self, content_hash: int) -> StatementResponse | None:
        """Load statement from database."""
        logger.debug(f"[STMT_MYSQL_LOAD] hash={content_hash}")
        data = self._load(content_hash)
        if data is None:
            logger.debug(f"[STMT_MYSQL_LOAD] NOT FOUND: hash={content_hash}")
            return None
    ...
    def load_statements_batch(
        ...
        logger.debug(f"[STMT_MYSQL_LOAD_BATCH] count={len(content_hashes)}")
    ...
    def delete_statement(self, content_hash: int) -> OperationResult[None]:
        ...
        logger.debug(f"[STMT_MYSQL_DELETE] hash={content_hash}")
