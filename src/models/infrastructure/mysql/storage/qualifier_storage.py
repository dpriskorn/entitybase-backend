"""Qualifier storage operations using Mysql."""
...
class QualifierMysqlStorage(BaseMysqlStorage):
    """Storage operations for qualifiers using Mysql."""
    ...
    def store_qualifier(
        ...
        logger.debug(f"[QUAL_MYSQL_STORE] hash={content_hash}")
        data = {
        ...
        if result.success:
            logger.debug(f"[QUAL_MYSQL_STORE] SUCCESS: hash={content_hash}")
        else:
            logger.error(f"[QUAL_MYSQL_STORE] FAILED: {result.error}")
        return result

    def load_qualifier(self, content_hash: int) -> S3QualifierData | None:
        """Load qualifier from database."""
        logger.debug(f"[QUAL_MYSQL_LOAD] hash={content_hash}")
        data = self._load(content_hash)
        if data is None:
            logger.debug(f"[QUAL_MYSQL_LOAD] NOT FOUND: hash={content_hash}")
            return None
    ...
    def load_qualifiers_batch(
        ...
        logger.debug(f"[QUAL_MYSQL_LOAD_BATCH] count={len(content_hashes)}")
    ...
    def delete_qualifier(self, content_hash: int) -> OperationResult[None]:
        ...
        logger.debug(f"[QUAL_MYSQL_DELETE] hash={content_hash}")
