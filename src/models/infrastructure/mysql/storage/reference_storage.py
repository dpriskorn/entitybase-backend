"""Reference storage operations using Mysql."""
...
class ReferenceMysqlStorage(BaseMysqlStorage):
    """Storage operations for references using Mysql."""
    ...
    def store_reference(
        ...
        logger.debug(f"[REF_MYSQL_STORE] hash={content_hash}")
        data = {
        ...
        if result.success:
            logger.debug(f"[REF_MYSQL_STORE] SUCCESS: hash={content_hash}")
        else:
            logger.error(f"[REF_MYSQL_STORE] FAILED: {result.error}")
        return result

    def load_reference(self, content_hash: int) -> S3ReferenceData | None:
        """Load reference from database."""
        logger.debug(f"[REF_MYSQL_LOAD] hash={content_hash}")
        data = self._load(content_hash)
        if data is None:
            logger.debug(f"[REF_MYSQL_LOAD] NOT FOUND: hash={content_hash}")
            return None
    ...
    def load_references_batch(
        ...
        logger.debug(f"[REF_MYSQL_LOAD_BATCH] count={len(content_hashes)}")
    ...
    def delete_reference(self, content_hash: int) -> OperationResult[None]:
        ...
        logger.debug(f"[REF_MYSQL_DELETE] hash={content_hash}")
