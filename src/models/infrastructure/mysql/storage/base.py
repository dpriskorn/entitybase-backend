"""Mysql storage for small objects (statements, qualifiers, references, snaks).
...
class BaseMysqlStorage(Repository):
    """Base class for Mysql storage operations."""
    ...
    def _store(
        ...
        try:
            ...
            logger.error(f"[MYSQL_STORE] Failed to store {self.table_name}: {e}")
    ...
    def _load(self, content_hash: int) -> dict[str, Any] | None:
        ...
        try:
            ...
            logger.error(f"[MYSQL_LOAD] Failed to load {self.table_name}: {e}")
            return None
    ...
    def _load_batch(self, content_hashes: list[int]) -> list[dict[str, Any] | None]:
        ...
        try:
            ...
            logger.error(f"[MYSQL_LOAD_BATCH] Failed: {e}")
            return [None] * len(content_hashes)
    ...
    def _delete(self, content_hash: int) -> OperationResult[None]:
        ...
        try:
            ...
            logger.error(f"[MYSQL_DELETE] Failed: {e}")
            return OperationResult(success=False, error=str(e))
