"""Snak storage operations using Mysql."""
...
class SnakMysqlStorage(BaseMysqlStorage):
    """Storage operations for snaks using Mysql."""
    ...
    def store_snak(
        ...
        logger.debug(f"[SNAK_MYSQL_STORE] hash={content_hash}")
        data = {
            "snak": snak_data.snak,
            "hash": content_hash,
        ...
        if result.success:
            logger.debug(f"[SNAK_MYSQL_STORE] SUCCESS: hash={content_hash}")
        else:
            logger.error(f"[SNAK_MYSQL_STORE] FAILED: {result.error}")
        return result

    def load_snak(self, content_hash: int) -> S3SnakData | None:
        """Load snak from database."""
        logger.debug(f"[SNAK_MYSQL_LOAD] hash={content_hash}")
        data = self._load(content_hash)
        if data is None:
            logger.debug(f"[SNAK_MYSQL_LOAD] NOT FOUND: hash={content_hash}")
            return None
    ...
    def load_snaks_batch(self, content_hashes: list[int]) -> list[S3SnakData | None]:
        ...
        logger.debug(f"[SNAK_MYSQL_LOAD_BATCH] count={len(content_hashes)}")
    ...
    def delete_snak(self, content_hash: int) -> OperationResult[None]:
        ...
        logger.debug(f"[SNAK_MYSQL_DELETE] hash={content_hash}")
