...
class ListingRepository:
    ...
    def list_locked(self, limit: int) -> list[dict]:
    ...
    def list_semi_protected(self, limit: int) -> list[dict]:
        conn = self.connection_manager.connect()
        cursor = conn.cursor()
        cursor.execute(
            """SELECT m.entity_id, h.head_revision_id
    ...
    def list_archived(self, limit: int) -> list[dict]:
        conn = self.connection_manager.connect()
        cursor = conn.cursor()
        cursor.execute(
            """SELECT m.entity_id, h.head_revision_id
    ...
    def list_dangling(self, limit: int) -> list[dict]:
        conn = self.connection_manager.connect()
        cursor = conn.cursor()
        cursor.execute(
            """SELECT m.entity_id, h.head_revision_id
