...
class WatchlistHandler(Handler):
    ...
    def add_watch(self, user_id: int, request: WatchlistAddRequest) -> MessageResponse:
        ...
        if not self.state.mysql_client.user_repository.is_watchlist_enabled(user_id):
            raise_validation_error(
                "Watchlist is disabled for this user", status_code=400
            )

        result = self.state.mysql_client.watchlist_repository.add_watch(
        ...
        # Update activity
        activity_result = self.state.mysql_client.user_repository.update_user_activity(
            user_id
        )
    ...
    def remove_watch(
        ...
        self.state.mysql_client.watchlist_repository.remove_watch(
            user_id, request.entity_id, request.properties
        )
    ...
    def remove_watch_by_id(self, user_id: int, watch_id: int) -> MessageResponse:
        ...
        result = self.state.mysql_client.watchlist_repository.remove_watch_by_id(
            watch_id
        )
        if not result.success:
            raise_validation_error(
    ...
    def get_watches(self, user_id: int) -> WatchlistResponse:
        ...
        watches = self.state.mysql_client.watchlist_repository.get_watches_for_user(
            user_id
        )
        # Update activity
        self.state.mysql_client.user_repository.update_user_activity(user_id)
    ...
    def get_notifications(
        ...
        notifications = (
            self.state.mysql_client.watchlist_repository.get_user_notifications(
    ...
    def mark_checked(
        ...
        self.state.mysql_client.watchlist_repository.mark_notification_checked(
    ...
    def get_watch_counts(self, user_id: int) -> WatchCounts:
        ...
        entity_count = (
            self.state.mysql_client.watchlist_repository.get_entity_watch_count(user_id)
        )
        property_count = (
            self.state.mysql_client.watchlist_repository.get_property_watch_count(
                user_id
            )
        )
