...
class UpdateTransaction(EntityTransaction):
    ...
    async def create_revision(
        ...
        logger.debug(
            f"[UpdateTransaction.create_revision] entity_id={entity_id}, db_client={id(self.state.db_client)}, id_resolver={id(self.state.db_client.id_resolver)}"
        )
    ...
    def _rollback_statement(self, hash_val: int) -> None:
        ...
        # Decrement ref_count
        self.state.db_client.decrement_ref_count(hash_val)
        # Check if orphaned and delete from S3
        ref_count = self.state.db_client.get_ref_count(hash_val)
        if ref_count == 0:
    ...
    def _rollback_revision(self, entity_id: str, revision_id: int) -> None:
        ...
        self.state.db_client.delete_revision(entity_id, revision_id)
