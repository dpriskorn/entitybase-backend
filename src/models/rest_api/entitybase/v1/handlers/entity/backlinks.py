...
class BacklinkHandler(Handler):
    ...
    async def get(
        ...
        internal_id = self.state.db_client.id_resolver.resolve_id(entity_id)
        ...
        backlinks = self.state.db_client.get_backlinks(internal_id, limit, offset)
        logger.debug(f"Got {len(backlinks)} backlinks")

        backlink_models = []
        for b in backlinks:
            referencing_entity_id = (
                self.state.db_client.id_resolver.resolve_entity_id(
