...
class EntityCreateHandler(EntityHandler):
    ...
    async def create_entity(
        ...
        entity_existed = self.state.db_client.entity_exists(entity_id)
        logger.debug(
            f"[create_entity] db_client={id(self.state.db_client)}, id_resolver={id(self.state.db_client.id_resolver)}, entity_existed={entity_existed}"
        )
        if entity_existed:
        ...
        self.state.db_client.register_entity(entity_id)

        # Check deletion status
        is_deleted = self.state.db_client.is_entity_deleted(entity_id)
        if is_deleted:
            raise_validation_error(
