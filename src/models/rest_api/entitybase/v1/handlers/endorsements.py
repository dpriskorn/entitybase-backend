...
class EndorsementHandler(Handler):
    ...
    async def endorse_statement(
        ...
        result = self.state.db_client.endorsement_repository.create_endorsement(
    ...
    async def withdraw_endorsement(
        ...
        result = self.state.db_client.endorsement_repository.withdraw_endorsement(
            user_id, statement_hash
        )
        if not result.success:
    ...
    def _get_and_validate_endorsement(
        ...
        endorsements_result = (
            self.state.db_client.endorsement_repository.get_statement_endorsements(
    ...
    def get_statement_endorsements(
        ...
        stats_result = self.state.db_client.endorsement_repository.get_batch_statement_endorsement_stats(
            [statement_hash]
        )
    ...
    def get_user_endorsements(
        ...
        if not self.state.db_client.user_repository.user_exists(user_id):  # type: ignore[union-attr]
            raise_validation_error("User not registered", status_code=404)

        result = self.state.db_client.endorsement_repository.get_user_endorsements(
    ...
    def get_user_endorsement_stats(self, user_id: int) -> EndorsementStatsResponse:
        ...
        result = (
            self.state.db_client.endorsement_repository.get_user_endorsement_stats(
                user_id
            )
        )
        if not result.success:
    ...
    def get_batch_statement_endorsement_stats(
        ...
        result = self.state.db_client.endorsement_repository.get_batch_statement_endorsement_stats(
            statement_hashes
        )
        if not result.success:
            raise_validation_error(
