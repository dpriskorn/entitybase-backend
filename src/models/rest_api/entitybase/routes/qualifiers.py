...
from models.rest_api.entitybase.response.qualifiers_references import QualifierResponse

logger = logging.getLogger(__name__)

qualifiers_router = APIRouter(prefix="/qualifiers", tags=["qualifiers"])
...
async def get_qualifiers(
    ...
    try:
        ...
        return [
            QualifierResponse(**item) if item is not None else None
            for item in result
        ]
