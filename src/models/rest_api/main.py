...
from .entitybase.response.endorsements import EndorsementResponse, EndorsementListResponse, EndorsementStatsResponse
...
@app.post("/entitybase/v1/statements/{statement_hash}/endorse", response_model=EndorsementResponse)
...
@app.delete("/entitybase/v1/statements/{statement_hash}/endorse", response_model=EndorsementResponse)
...
@app.get("/entitybase/v1/statements/{statement_hash}/endorsements", response_model=EndorsementListResponse)
def get_statement_endorsements(
    statement_hash: int,
    limit: int = Query(50, ge=1, le=500, description="Maximum number of endorsements to return"),
    ...
    if not isinstance(clients, Clients):
        raise_validation_error("Invalid clients type", status_code=500)
    handler = EndorsementHandler()
    request = EndorsementListRequest(limit=limit, offset=offset, include_removed=include_removed)
...
@app.get("/entitybase/v1/users/{user_id}/endorsements", response_model=EndorsementListResponse)
...
@app.get("/entitybase/v1/users/{user_id}/endorsements/stats", response_model=EndorsementStatsResponse)
