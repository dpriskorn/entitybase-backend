...
class UserActivityItemResponse(BaseModel):
    ...
    entity_id: str = Field(
        default="", description="Entity ID associated with the activity"
    )
    revision_id: int = Field(
        default=0, description="Revision ID associated with the activity"
    )
...
class UserActivityResponse(BaseModel):
    ...
    activities: List[UserActivityItemResponse] = Field(
